package com.evdelacruz.samples.beanvalidation.util.error;
...
public abstract class ErrorHandler<R> {
    ...
    public R handleError(Exception ex) {
        if (responses.containsKey(ex.getClass())) {
            logger.debug("Excepci\u00f3n capturada durante procesamiento de una acci\u00f3n", ex);
            return responses.get(ex.getClass()).apply(ex);
        }
        logger.error("Excepci\u00f3n no esperada durante procesamiento de una acci\u00f3n", ex);
